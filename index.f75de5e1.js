...
tbodies.forEach((tb)=>{
    ...
    for(let i = 0; i < tds.length; i++)row[theads[i].innerText] = tds[i].innerText;
    rows.push(row);
});
header.addEventListener("click", (e)=>{
    ...
    if (head) {
        ...
        let newRows = [
            ...rows
        ].sort((a, b)=>compareTwoStrings(a[mainKey], b[mainKey]));
        if (mainKey.toLowerCase() === "age" || mainKey.toLowerCase() === "salary") newRows = [
            ...newRows
        ].sort((a, b)=>{
